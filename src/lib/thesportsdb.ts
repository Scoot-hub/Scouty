...
const CLUB_NAME_MAP: Record<string, string[]> = {
  ...
  'LOSC Lille': ['Lille', 'LOSC Lille', 'LOSC'],
  'Stade Rennais': ['Rennes', 'Stade Rennais', 'SRFC'],
  'OGC Nice': ['OGC Nice', 'Nice', 'OGCN'],
  'RC Lens': ['RC Lens', 'Lens', 'RCL'],
  'Stade Brestois': ['Stade Brestois', 'Brest', 'SB29'],
  'RC Strasbourg': ['RC Strasbourg', 'Strasbourg', 'RCSA'],
  'Montpellier HSC': ['Montpellier', 'MHSC'],
  'FC Nantes': ['FC Nantes', 'Nantes', 'FCN'],
  'Toulouse FC': ['Toulouse', 'TFC'],
  'Stade de Reims': ['Stade de Reims', 'Reims', 'SDR'],
  ...
  'FC Lorient': ['FC Lorient', 'Lorient', 'FCL'],
  'Amiens SC': ['Amiens'],
  'Grenoble Foot': ['Grenoble Foot 38', 'Grenoble', 'GF38'],
  'Rodez AF': ['Rodez'],
  'USL Dunkerque': ['Dunkerque'],
  'SC Bastia': ['Bastia', 'SC Bastia'],
  'Red Star FC': ['Red Star FC', 'Red Star'],
  'FC Martigues': ['FC Martigues'],
  'Girondins de Bordeaux': ['Bordeaux', 'Girondins', 'FCGB'],
  'FC Sochaux': ['Sochaux', 'FCSM'],
  'Clermont Foot': ['Clermont', 'CF63'],
  'Dijon FCO': ['Dijon', 'DFCO'],
  'EA Guingamp': ['Guingamp', 'EAG'],
  'Chamois Niortais': ['Niort', 'Chamois Niortais'],
  'US Quevilly-Rouen': ['QRM', 'Quevilly Rouen'],
  'AC Ajaccio': ['AC Ajaccio', 'ACA', 'Ajaccio'],
  'Pau FC': ['Pau FC', 'Pau'],
  'FC Valenciennes': ['Valenciennes', 'VAFC'],
  'AS Nancy': ['Nancy', 'ASNL'],
  ...
  'Atlético de Madrid': ['Atletico Madrid', 'Atletico', 'Atleti'],
  ...
  // Allemagne
  'Bayern Munich': ['Bayern Munich', 'FC Bayern Munich', 'Bayern', 'FCB München'],
...
// Strict fuzzy match: only accept "a contains b" (or vice-versa) when the shorter
// string is at least 5 chars AND appears as a whole word in the longer one.
// Without this, short aliases like "NEC" falsely match "FC Annecy" (an-NEC-y).
function fuzzyClubMatch(a: string, b: string): boolean {
  if (!a || !b) return false;
  if (a === b) return true;
  const [shorter, longer] = a.length <= b.length ? [a, b] : [b, a];
  if (shorter.length < 5) return false;
  const escaped = shorter.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return new RegExp(`(^|\\s)${escaped}(\\s|$)`, 'i').test(longer);
}
...
export async function searchClubLogo(clubName: string): Promise<ClubInfo | null> {
  ...
  for (const [mapKey, alts] of Object.entries(CLUB_NAME_MAP)) {
    ...
    if (fuzzyClubMatch(normalizedKey, normalizedInput)) {
      for (const alt of alts) {
        const result = await trySearch(alt);
        if (result?.badge_url) return result;
      }
    ...
    const aliasMatch = alts.some(alt => {
      ...
      return fuzzyClubMatch(normalizedAlt, normalizedInput);
    });
    if (aliasMatch) {
      for (const alt of alts) {
        const result = await trySearch(alt);
